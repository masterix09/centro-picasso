...
export async function onSubmitCreatePaziente(
...
// 2. Define a submit handler.
export function onSubmitCreatePrestazione(
  values: z.infer<typeof formSchema>,
  idPiano: string,
  idDente: string
) {
  ...
  return addPrestazionePianoCura(
    values.prestazioni,
    idPiano,
    idDente,
    values.sede,
...
// 2. Define a submit handler.
export function onSubmitCreateDocumento(
  values: z.infer<typeof formSchema>,
  idPiano: string,
  idDente: string
) {
...
export async function onSubmitUpdatePaziente(
  values: z.infer<typeof formSchema>,
  idCliente: string
) {
  const {
  ...
  const res = await updatePaziente(
    nome,
    cognome,
    dataNascita,
    sesso,
  ...
  if (res === "ok") {
    ...
    toast({
      variant: "destructive",
      title: "Uh Oh! Errore nell aggiornamento.",
      description: "Errore nella aggiornamento del paziente. Riprova",
    });
  }
}
